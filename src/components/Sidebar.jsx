...
const Sidebar = ({ homeRef, aboutRef, contactRef, xRef, oneRef, twoRef, threeRef, scrollToSection }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      } else if (scrollPosition >= contactRef.current.offsetTop && scrollPosition < xRef.current.offsetTop) {
        setActiveSection(contactRef);
      } else if (scrollPosition >= xRef.current.offsetTop && scrollPosition < oneRef.current.offsetTop) {
